package handlers

import (
	"fmt"
	"net/http"
	"testing"

	"github.com/doncicuto/glim/common"
)

func TestRefresh(t *testing.T) {
	// Setup
	h, e, settings := testSetup(t, false)
	defer testCleanUp()

	// Log in with admin user and get tokens
	adminToken, refreshToken := getUserTokens("admin", h, e, settings)
	_, plainUserRefreshToken := getUserTokens("saul", h, e, settings)

	// Test cases
	testCases := []RestTestCase{
		{
			name:             "Bad request expired token",
			expResCode:       http.StatusBadRequest,
			reqURL:           refreshTokenEndpoint,
			reqBodyJSON:      `{"refresh_token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..1DZfzMDf2jtaVQBFXOmimFpdauuBdoTFcF2N-BNc0sg"}`,
			reqMethod:        http.MethodPost,
			expectedBodyJSON: fmt.Sprintf(`{"message":"%s"}`, common.CouldNotParseTokenMessage),
		},
		{
			name:             "Bad request uid not found in token",
			expResCode:       http.StatusBadRequest,
			reqURL:           refreshTokenEndpoint,
			reqBodyJSON:      `{"refresh_token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..ifF_FxTdMbzVoAesbIPayKnm9W9KF3jbiAAIILah7JY"}`,
			reqMethod:        http.MethodPost,
			expectedBodyJSON: fmt.Sprintf(`{"message":"%s"}`, common.UidNotFoundInTokenMessage),
		},
		{
			name:             "Bad request jti not found in token",
			expResCode:       http.StatusBadRequest,
			reqURL:           refreshTokenEndpoint,
			reqBodyJSON:      `{"refresh_token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..mP8ZCNtI6_tx8JnFSzq--ossC9aUUh584vchAfLq0Dw"}`,
			reqMethod:        http.MethodPost,
			expectedBodyJSON: fmt.Sprintf(`{"message":"%s"}`, common.JTINotFoundInTokenMessage),
		},
		{
			name:             "Bad request access jti not found in token",
			expResCode:       http.StatusBadRequest,
			reqURL:           refreshTokenEndpoint,
			reqBodyJSON:      `{"refresh_token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..5j3Sfwks_4fRtRgcZYVU-sGmBpvClSP9nWxhlKCXCNU"}`,
			reqMethod:        http.MethodPost,
			expectedBodyJSON: fmt.Sprintf(`{"message":"%s"}`, common.JTINotFoundInTokenMessage),
		},
		{
			name:             "Bad request access iat not found in token",
			expResCode:       http.StatusBadRequest,
			reqURL:           refreshTokenEndpoint,
			reqBodyJSON:      `{"refresh_token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..4TaxwUmi5riq90RRCzvg7CsrBJHxLcbmYalSSmCQULM"}`,
			reqMethod:        http.MethodPost,
			expectedBodyJSON: fmt.Sprintf(`{"message":"%s"}`, common.IatNotFoundInTokenMessage),
		},
		{
			name:             "Unauthorized expired refresh token",
			expResCode:       http.StatusUnauthorized,
			reqURL:           refreshTokenEndpoint,
			reqBodyJSON:      `{"refresh_token": "eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9..T7FIZembax4xD3zozT_9fbEeWsPbJAmG4VkLFl1Fsmk"}`,
			reqMethod:        http.MethodPost,
			expectedBodyJSON: fmt.Sprintf(`{"message":"%s"}`, common.RefreshTokenUsageWithoutLogInMessage),
		},
		{
			name:       "user deleted",
			expResCode: http.StatusNoContent,
			reqURL:     "/v1/users/3",
			reqMethod:  http.MethodDelete,
			secret:     adminToken,
		},
		{
			name:             "Bad request invalid uid found in token",
			expResCode:       http.StatusBadRequest,
			reqURL:           refreshTokenEndpoint,
			reqBodyJSON:      fmt.Sprintf(`{"refresh_token": "%s"}`, plainUserRefreshToken),
			reqMethod:        http.MethodPost,
			expectedBodyJSON: fmt.Sprintf(`{"message":"%s"}`, common.InvalidUIDInTokenMessage),
		},
		{
			name:        "Refreshed token successful",
			expResCode:  http.StatusOK,
			reqURL:      refreshTokenEndpoint,
			reqBodyJSON: fmt.Sprintf(`{"refresh_token": "%s"}`, refreshToken),
			reqMethod:   http.MethodPost,
		},
		{
			name:             "Unauthorized refresh token blacklisted",
			expResCode:       http.StatusUnauthorized,
			reqURL:           refreshTokenEndpoint,
			reqBodyJSON:      fmt.Sprintf(`{"refresh_token": "%s"}`, refreshToken),
			reqMethod:        http.MethodPost,
			expectedBodyJSON: fmt.Sprintf(`{"message":"%s"}`, common.TokenNoLongerValidMessage),
		},
	}

	for _, tc := range testCases {
		runTests(t, tc, e)
	}
}
